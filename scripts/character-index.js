...
const CHARACTER_DIR = path.resolve(__dirname, '../characters');
...
function ensureOptionalStringList(value, fieldName, canonicalId) {
    ...
    if (!Array.isArray(value)) {
        throw new Error(`characters/${canonicalId}.json: ${fieldName} must be an array of strings.`);
    }
...
async function loadCharacterIndex() {
    try {
        const directoryEntries = await fs.readdir(CHARACTER_DIR, { withFileTypes: true });
